...
public class UnloadPlanner {
    // Only for unit test
    public static boolean enableSingleReplicationShuffle = false;
    private boolean shuffleServiceEnable = false;

    private static final Logger LOG = LogManager.getLogger(UnloadPlanner.class);

    public ExecPlan plan(UnloadStmt unloadStmt, ConnectContext session) {
        QueryRelation queryRelation = unloadStmt.getQueryStatement().getQueryRelation();
        List<ColumnRefOperator> outputColumns = new ArrayList<>();
        Table targetTable = unloadStmt.getTargetTable();
        ...
        if (queryRelation instanceof ValuesRelation) {
            castLiteralToTargetColumnsType(unloadStmt);
        }
        ...
        OptExprBuilder optExprBuilder = fillDefaultValue(logicalPlan, columnRefFactory, unloadStmt, outputColumns);
        ...
        optExprBuilder = fillMaterializedColumns(columnRefFactory, unloadStmt, outputColumns, optExprBuilder, session);

        //6. Fill in the shadow column
        optExprBuilder = fillShadowColumns(columnRefFactory, unloadStmt, outputColumns, optExprBuilder, session);

        //7. Cast output columns type to target type
        optExprBuilder =
                castOutputColumnsTypeToTargetColumns(columnRefFactory, unloadStmt, outputColumns, optExprBuilder);
        ...
        try (PlannerProfile.ScopedTimer ignore = PlannerProfile.getScopedTimer("UnloadPlanner")) {
            ...
            PhysicalPropertySet requiredPropertySet = createPhysicalPropertySet(unloadStmt, outputColumns);
            ...
            DataSink dataSink;
            if (unloadStmt.getTargetTable() instanceof MysqlTable) {
                ...
                dataSink = new IcebergTableSink((IcebergTable) targetTable, tupleDesc,
                        isKeyPartitionStaticUnload(unloadStmt, queryRelation));
            } else if (targetTable instanceof TableFunctionTable) {
                dataSink = new TableFunctionTableSink((TableFunctionTable) targetTable);
            } else {
                throw new SemanticException("Unknown table type " + unloadStmt.getTargetTable().getType());
            }
    ...
    private void castLiteralToTargetColumnsType(UnloadStmt insertStatement) {
    ...
    private OptExprBuilder fillDefaultValue(LogicalPlan logicalPlan, ColumnRefFactory columnRefFactory,
                                            UnloadStmt insertStatement, List<ColumnRefOperator> outputColumns) {
    ...
    private OptExprBuilder fillMaterializedColumns(ColumnRefFactory columnRefFactory, UnloadStmt insertStatement,
        ...
        for (int columnIdx = 0; columnIdx < fullSchema.size(); ++columnIdx) {
            ...
            if (targetColumn.isMaterializedColumn()) {
                // If fe restart and Unload INTO is executed, the re-analyze is needed.
    ...
    private OptExprBuilder fillShadowColumns(ColumnRefFactory columnRefFactory, UnloadStmt insertStatement,
                                             List<ColumnRefOperator> outputColumns, OptExprBuilder root,
                                             ConnectContext session) {
        Set<Column> baseSchema = Sets.newHashSet(insertStatement.getTargetTable().getBaseSchema());
        List<Column> fullSchema = insertStatement.getTargetTable().getFullSchema();
     ...
     */
    private PhysicalPropertySet createPhysicalPropertySet(UnloadStmt unloadStmt,
                                                          List<ColumnRefOperator> outputColumns) {
        QueryRelation queryRelation = unloadStmt.getQueryStatement().getQueryRelation();
        ...
        if (unloadStmt.getTargetTable() instanceof IcebergTable) {
            IcebergTable icebergTable = (IcebergTable) unloadStmt.getTargetTable();
        ...
        if (!(unloadStmt.getTargetTable() instanceof OlapTable)) {
            return new PhysicalPropertySet();
        }

        OlapTable table = (OlapTable) unloadStmt.getTargetTable();
    ...
    private boolean isKeyPartitionStaticUnload(UnloadStmt unloadStmt, QueryRelation queryRelation) {
        ...
        if (unloadStmt.getTargetColumnNames() == null) {
            targetColumnNames = icebergTable.getColumns().stream()
                    .map(Column::getName).collect(Collectors.toList());
        } else {
            targetColumnNames = Lists.newArrayList(unloadStmt.getTargetColumnNames());
        }
